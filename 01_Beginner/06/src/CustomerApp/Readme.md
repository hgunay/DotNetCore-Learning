...
if statement verilen bir koşulu kontrol eder ve eğer koşul `true` olarak değerlendirilirse if bloğu içerisindeki yazılan tüm kod çalıştırılır. if statement için basit bir örneği bir önceki bölümde if-else statement kullanarak **Null Reference Exception** hatasını kontrol ederken kullanmıştık. Ancak tekrar hatırlatmakta fayda var :)
...
Daha önce **[array](https://github.com/hgunay/DotNetCore-Learning/tree/master/01_Beginner/03/src/CustomerApp#arrays-diziler)** başlığı altında index'ler 0'dan başlar ve tanımlı olduğu boyut kadar arttırılabilir demiştik. İşte burada da aynı durum geçerli.
...
Kullanım olarak ise **if statement**'a çok benzer ve ikisi de belirtilen koşul `true` olduğu sürece içerisinde yazılan kod bloğunu çalıştırır ancak while döngüsünde bu kod bloğu koşul `false` oluncaya kadar sürekli döngüde kalarak çalıştırılmaya devam eder.
...
> `break` komutunu özellikle **[switch](https://github.com/hgunay/DotNetCore-Learning/tree/master/01_Beginner/05/src/CustomerApp#switch-statement)** yazarken bolca kullanıyor olacaksınız.  
